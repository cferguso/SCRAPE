                ...
                for a in links:
                    ...
                    if link.find('/') != -1:
                        ...
                        if spl[1] in content or spl[3] in content:
                            ...
                            if requester == 'top level' or (requester == 'second level' and (link.find(".") > 0 or link.find("&ext=") > 0)):
                                try:
                                    time.sleep(1)
...
import requests, os, re, time, pandas as pd
from bs4 import BeautifulSoup
        ...
        if topLevel:
            ...
            if len(topval) > 0:
                ...
                for nexturl in topval:
                    ...
                    nextsoup, nextdest = my_soup(nexturl, root)
                
                    nextlevel, nextval = scrape_files("second level", nextsoup, nextdest, dig=False)
